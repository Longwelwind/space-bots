...
The maximum number of ships a fleet can have is equal to the highest `leadershipValue` of all ships that composes
this fleet.
For example, having one `admiral_mk_1` (which has a `leadershipValue` of 11) in a fleet allows it to have 11 ships.
Having multiple `admiral_mk_1` in your fleet will not increase the ship limit.

The base ship limit is 5.
